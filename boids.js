...
class Boid {
  ...
  flock(boids, obstacles, vehicles) {
    let alignment = this.align(boids);
    let cohesion = this.cohesion(boids);
    let separation = this.separation(boids);
    let avoidance = this.avoid(obstacles);
    let fleeFromVehicles = this.fleeVehicles(vehicles);
  
    alignment.mult(this.alignWeight);
    cohesion.mult(this.cohesionWeight);
    separation.mult(this.separationWeight);
    avoidance.mult(2); // Ajustez ce poids pour l'évitement
    fleeFromVehicles.mult(3);     // Poids pour fuir les véhicules
    ...
    this.applyForce(fleeFromVehicles);
  ...
  avoid(obstacles) {
    let desiredAvoidance = createVector(0, 0);
  
    obstacles.forEach(obstacle => {
      let distance = this.pos.dist(obstacle.pos);
  
      if (distance < this.perceptionRadius + obstacle.r) {
        let force = p5.Vector.sub(this.pos, obstacle.pos);
        force.setMag(this.maxSpeed);
        force.sub(this.vel);
        force.limit(this.maxForce);
        desiredAvoidance.add(force);
      }
    });
  ...
  stayWithinBounds(boundaryPadding = 25) {
let desired = null;

if (this.pos.x < boundaryPadding) {
  desired = createVector(this.maxSpeed, this.vel.y);
} else if (this.pos.x > width - boundaryPadding) {
  desired = createVector(-this.maxSpeed, this.vel.y);
}

if (this.pos.y < boundaryPadding) {
  desired = createVector(this.vel.x, this.maxSpeed);
} else if (this.pos.y > height - boundaryPadding) {
  desired = createVector(this.vel.x, -this.maxSpeed);
}
...
vehicles.forEach(vehicle => {
  ...
  if (distance < this.perceptionRadius + vehicle.r) {
    let diff = p5.Vector.sub(this.pos, vehicle.pos);
    diff.normalize();
    diff.div(distance); // Plus proche = plus forte la force
    fleeForce.add(diff);
    total++;
  }
});
